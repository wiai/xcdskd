    ...
    def _parseHeader(self, lines):
        ...
        for i, line in enumerate(lines):
            ...
            if line[0] == 'Author':
                ...
                self._acquisitionEuler1 = int(float(line[1]))
            elif line[0] == 'AcqE2':
                self._acquisitionEuler2 = int(float(line[1]))
            elif line[0] == 'AcqE3':
                self._acquisitionEuler3 = int(float(line[1]))
            elif line[0] == 'Euler angles refer to Sample Coordinate system (CS0)!':
                for j, item in enumerate(line[1:]):
                    if item == 'Mag':
                        self._magnification = int(float(line[j + 2]))
                    elif item == 'Coverage':
                        self._coverage = int(float(line[j + 2]))
                    elif item == 'Device':
                        self._device = int(float(line[j + 2]))
                    elif item == 'KV':
                        self._acceleratingVoltage = float(line[j + 2])
                    elif item == 'TiltAngle':
                        self._tiltAngle = float(line[j + 2])
                    elif item == 'TiltAxis':
                        self._tiltAxis = int(float(line[j + 2]))
            elif line[0] == 'Phases':
                self._numberPhases = int(float(line[1]))
...
if __name__ == '__main__': #pragma: no cover
    ctf = ctf('Map Data 1.ctf')
    print(len(ctf.getPixelArray(key='bc', phase=('=', 2), bc=('>', 50))))
    print(ctf.getPixelArray(key='bc', phase=('=', 1), bc=('>', 10)))
